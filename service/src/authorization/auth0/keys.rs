...
impl Keys {
    ...
    ///
    /// # Parameters
    /// - `alg` - The algorithm of the key to retrieve.
    /// - `kid` - The ID of the key to retrieve.
    ///
    /// # Returns
    /// The matching key, if one could be found.
    #[tracing::instrument(skip(self))]
    pub async fn get(&self, alg: SignatureAlgorithm, kid: &str) -> Option<JWK<()>> {
        let result = self
            .client
...
mod tests {
    ...
    async fn get_key_success() {
        ...
        let key = sut.get(jwa::SignatureAlgorithm::RS256, "myKeyId").await;
